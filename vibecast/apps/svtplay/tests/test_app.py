    ...
    *,
    ...
    return AppContext(
        ...
        receiver=ReceiverContext(
            ...
            data_dir=Path("/tmp/vibecast-tests/apps/svtplay"),
        ),
...
class TestProperties:
    def test_app_metadata(self) -> None:
        app = SvtPlay()
        assert app.app_ids() == frozenset({"95370A1C"})
        assert app.display_name() == "SVT Play"
...
class TestLifecycle:
    async def test_on_launch_and_stop_manage_session(self) -> None:
        ...
        async with httpx.AsyncClient(transport=httpx.MockTransport(_handler)) as client:
            ...
            assert session.session_id in app._sessions  # noqa: SLF001

            await app.on_stop(session)
            assert session.session_id not in app._sessions  # noqa: SLF001
