...
from vibecast.apps.viaplay._api import (
    DeviceAuthInfo,
    SessionCheckResult,
    StreamInfo,
)
from vibecast.apps.viaplay._app import _NS_VIAPLAY, Viaplay
...
def _make_session(
    ...
    app_id: str = "6313CF39",
) -> tuple[AppContext, AsyncMock, AsyncMock]:
    ...
    session = AppContext(
        ...
        receiver=ReceiverContext(
            ...
            data_dir=Path("/tmp/vibecast-tests/apps/viaplay"),
        ),
...
class TestProperties:
    def test_app_metadata(self) -> None:
        app = Viaplay()
        assert app.app_ids() == frozenset({"6313CF39", "2DB7CC49"})
        assert app.display_name() == "Viaplay"
        assert _NS_VIAPLAY in app.namespaces()
...
class TestLifecycle:
    async def test_on_launch_and_stop_manage_session(self) -> None:
        ...
        await app.on_launch(session, LaunchCredentials(credentials="token"))
        assert "sess-1" in app._sessions  # noqa: SLF001

        await app.on_stop(session)
        assert "sess-1" not in app._sessions  # noqa: SLF001
    ...
    async def test_setup_info_starts_auth_flow_task(self) -> None:
        ...
        with patch.object(app, "_run_auth_flow", new_callable=AsyncMock) as mock_auth:
            ...
            state = app._sessions["sess-1"]  # noqa: SLF001
...
class TestAuthFlowEdges:
    async def test_start_device_auth_emits_authorization_required(self) -> None:
        ...
        with (
            ...
            patch.object(app, "_poll_for_authorization", new_callable=AsyncMock),
        ):
            await app._start_device_auth(session, state, SessionCheckResult())
